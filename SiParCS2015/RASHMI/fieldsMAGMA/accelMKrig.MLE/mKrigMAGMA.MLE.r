mKrigMAGMA.MLE <- function(x, y, weights = rep(1, nrow(x)), 
    ...
    # these are all the arguments needed to call mKrigMAGMA except lambda and those in par.grid.
    ...
    temp.fn <- function(x) {
        ...
        hold <- do.call("mKrigMAGMA", c(mKrig.args, list(find.trA = FALSE), 
    ...
    for (k in 1:NG) {
        ...
        obj <- do.call("mKrigMAGMA", c(mKrig.args, cov.args.temp, 
