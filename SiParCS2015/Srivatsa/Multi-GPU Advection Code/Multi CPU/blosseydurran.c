...
void local_odefun(float time_step, real *temp_sol, real *step_sol);
void build_local_diff_matrices();
...
void partition_domain(char *partition_file, char *neighbor_file)
{
	...
	/* determine the eval nodes */

	internal_node_count = 0;

	for (j = 0; j < NUM_NODES; j++)
	{
		if(global_nodes[j].partition_id == rank && global_nodes[j].ghost_flag == GHOST_FLAG_I)
			internal_node_count++;
	}
	...
	{
		if(global_nodes[j].partition_id == rank && global_nodes[j].ghost_flag == GHOST_FLAG_I)
			internal_node_list[i_count++] = global_nodes[j].global_id;
	}
...
void build_receive_buffers()
{
	...
	boundary_ghost_nodes_receive_buffer = (int **)calloc(neighbor_procs_receive_count, sizeof(*boundary_ghost_nodes_receive_buffer));
	...
	for(i = 0; i < neighbor_procs_receive_count; i++)
	{
		...
		boundary_ghost_nodes_receive_buffer[i] = (int *)calloc(boundary_ghost_partition_counters_receive[pid], sizeof(int));
	}	
	...
	for(i = 0; i < neighbor_procs_receive_count; i++)
	{
		...
		for(j = 0; j < vector_total(&boundary_ghost_nodes_receive[pid]); j++)
		{
			boundary_ghost_nodes_receive_buffer[i][j] = (int)vector_get(&boundary_ghost_nodes_receive[pid],j);
		}
	}
...
void build_send_buffers()
{
	...
	boundary_ghost_nodes_send_buffer = (int **)calloc(neighbor_procs_send_count, sizeof(*internal_ghost_nodes_send_buffer));
	...
	for(i = 0; i < neighbor_procs_send_count; i++)
	{
		...
		boundary_ghost_nodes_send_buffer[i] = (int *)calloc(boundary_ghost_partition_counters_send[pid], sizeof(int));
	}
...
void exchange_receive_indexes()
{
	...
	/* Exchange internal node indexes */
	...
	/* Exchange boundary node indexes */
	...
	{
		int pid = neighbor_procs_send[i];
      	MPI_Irecv(boundary_ghost_nodes_send_buffer[i], boundary_ghost_partition_counters_send[pid], MPI_INT, pid, 0,
              MPI_COMM_WORLD, &receive_request[i]);      	
    }
	...
	{
		int pid = neighbor_procs_receive[i];
		MPI_Isend(boundary_ghost_nodes_receive_buffer[i], boundary_ghost_partition_counters_receive[pid], MPI_INT, pid, 0,
              MPI_COMM_WORLD, &send_request[i]);		
	}
...
void pde_solver()
{
	...
	for (i = 1; i <= 1; i++)
	{
		...
		local_odefun(time_steps[i - 1], temp_sol, step_1_sol);		
...
void build_local_diff_matrices()
{
	...
	local_Wx = (real *)malloc(internal_node_count * STENCIL_SIZE * sizeof(real));
	local_Wy = (real *)malloc(internal_node_count * STENCIL_SIZE * sizeof(real));
	local_GammaHV = (real *)malloc(internal_node_count * STENCIL_SIZE * sizeof(real));

	for(i = 0; i < internal_node_count; i++)
	{
		id  = internal_node_list[i] - 1;

		for(j = 0; j < STENCIL_SIZE; j++)
		{
			local_Wx[i * STENCIL_SIZE + j] = Wx->val[id][j];
			local_Wy[i * STENCIL_SIZE + j] = Wy->val[id][j];
			local_GammaHV[i * STENCIL_SIZE + j] = GammaHV[id][j];
		}
	}
...
void local_odefun(float time_step, real *temp_sol, real *step_sol)
{
	...
	for (i = 0; i < internal_node_count; i++)
	{
		int node_id = internal_node_list[i] - 1;
		...
		for (j = 0; j < STENCIL_SIZE; j++)
		{
			index = nodes->neighbors[node_id][j] - 1;
			val = temp_sol[index];
			x_sol[node_id] += local_Wx[i * STENCIL_SIZE + j] * val;
			y_sol[node_id] += local_Wy[i * STENCIL_SIZE + j] * val;
			hv_sol[node_id] += local_GammaHV[i * STENCIL_SIZE + j] * val;
		}

		step_sol[node_id] = ((-1) * nodes->u[node_id] * x_sol[node_id]) + ((-1) * nodes->v[node_id] * y_sol[node_id]) + hv_sol[node_id];
	}
	end_time = get_time();
	if(rank == 0)	
		printf("Time elapsed eval_rhs: %llu\n", end_time - start_time);
