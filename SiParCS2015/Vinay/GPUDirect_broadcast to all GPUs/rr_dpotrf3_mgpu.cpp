        ...
        for (j=0; j < m; j += nb) {
            ...
            if ( (j+jb) < n) {
                ...
                if ( j+jb < m ) {
                    ...
                    //                        queues[d][stream3] );
                    buf2 = ((j+jb)/nb)%ngpu;
                    //magma_dcopymatrix_async( j+jb, nb0,
                    //                         dlA(d,0,nb*j_local2),ldda,
                    //                         dlP((d+1)%2,nb,0,buf2), lddp,
                    //                         queues[d][stream3] );
                    for(d2=0; d2< ngpu; d2++)
		    {
			if ( d2 != d ) {
		    ...
		    //cudaMemcpyPeerAsync(dlP((d+1)%2,nb,0,buf2),(d+1)%2,dlA(d,0,nb*j_local2),d,(j+jb)*nb0*sizeof(double),queues[d][stream3]);
		    trace_gpu_end( d, stream3 );
			    ...
			    magma_queue_sync(queues[d2][stream3]);
                        }
                    }
